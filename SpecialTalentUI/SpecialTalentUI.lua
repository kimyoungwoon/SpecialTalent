...
function SpecialTalentFrame_Minimize()
	...
	SpecialTalentFrame:SetHeight(706);
	for i=2, 4 do
			...
			for i=1, MAX_NUM_TALENTS do
				...
				if ( i <= numTalents ) then
					...
					elseif ( SpecialTalentFrame.learnMode == "planned" ) then
						...
						talentPoints = TALENT_POINTS_AT_MAX_LEVEL - SpecialTalentPlannedSaved[player].points;
						rank = planRank;
					end
...
-- ST memo -- update for selected player
	...
	planned = planned.." = "..(SpecialTalentPlannedSaved[player].points or 0).."/"..TALENT_POINTS_AT_MAX_LEVEL;
...
function SpecialTalentFrame_SetPrereqs(...)
	local buttonTier = select(1,...);
	local buttonColumn = select(2,...);
	local forceDesaturated = select(3,...);
	local tierUnlocked = select(4,...);
	local tab = select(5,...);
...
function SpecialTalent_PlanTalent( tabID, talentID )
	...
	if ( talentPoints<TALENT_POINTS_AT_MAX_LEVEL ) then
...
function SpecialTalent_GetTalentInfo(tabID, talentID, class)
	...
	if ( PlayerOfRealm and (PlayerOfRealm == ActivePlayerOfRealm) and tabID and talentID ) then
...
function SpecialTalentButton_OnEnter()
	...
	if ( not (PlayerOfRealm == ActivePlayerOfRealm) ) then
		...
		if ( maxRank == 1 ) then
			...
			-- 마나소모 타잎 판별 07.4.5 Loveable(Deblan)
			local manatype

		if (ST_PlayerClass == "WARRIOR" ) then
			manatype = "분노"
		elseif (ST_PlayerClass == "ROGUE") then
			manatype = "기력"
		else
			manatype = "마나"
		end
			...
			GameTooltip:AddLine(manatype.." "..mana, 1.0, 1.0, 1.0);
			-- 마나소모 타잎 판별 구문 끝
			end
...
function SpecialTalentFrame_LoadServerPlayer()
	...
	for value in pairs(SpecialTalentPlannedSaved) do
--		local player = gsub(value, "(%a+) of .*", "%1");
--		local server = gsub(value, "%a+ of (.*)", "%1");
		local _,_,player,server = string.find(value, "([^%s]+)%s+of%s+(.+)");
...
function SpecialTalentFrameServerDropDown_Initialize()
	local info;
	for server in pairs(SPECIAL_TALENT_SERVER) do
		info = {};
		info.text = server;
...
function SpecialTalentFrameServerDropDown_GetID( server )
	local i=1;
	for key in pairs(SPECIAL_TALENT_SERVER) do
		if ( key==server ) then
			return i;
		end
		i=i+1;
...
function SpecialTalentFrameAltDropDown_Initialize()
	local info;
	for player in pairs(SPECIAL_TALENT_SERVER[SpecialTalentFrameServerDropDown.server]) do
		info = {};
		info.text = player;
...
function SpecialTalentFrameAltDropDown_GetID( player )
	local i=1;
	for key in pairs(SPECIAL_TALENT_SERVER[SpecialTalentFrameServerDropDown.server]) do
		if ( key==player ) then
			return i;
		end
		i=i+1;
